...
export default function EventComp({ theme, eventData }: EventCompProps) {
  ...
  const handleMouseEnter = () => {
    ctx.add(() => {
      gsap.to(gradRef.current, {
        background:
          "linear-gradient(to bottom, rgba(30, 28, 28,.2) , rgba(30, 28, 28, .9))",
      });
  ...
  const handleMouseLeave = () => {
    ...
    ctx.add(() => {
      gsap.to(gradRef.current, {
        background:
          "linear-gradient(to bottom, rgba(30, 28, 28,.2) 20%, rgba(30, 28, 28, 1))",
      });
  ...
  return (
    ...
    >
      ...
      <div className="component_main" ref={mainRef}>
        <div className="component_main_content">
          ...
          <div className="component_main_info">
            <p>
              {formatDate(eventData.date)[0]} {formatDate(eventData.date)[1]}{" "}
            </p>
            <div className="component_main_statusBar">
              <span
                className="dot"
                style={{ background: dotColor[eventData.status] }}
              ></span>
      ...
      <div className="component_desc" ref={descRef}>
        <div>
          <h2 className="component_main_header" style={{ marginBottom: "1%" }}>
            {eventData.name}
          </h2>
          {eventData.desc.length > 80 ? (
            <>
              <p>
                {eventData.desc.slice(0, 80)}
                {content === "less" ? (
                  <span>
                    {eventData.desc.slice(81, eventData.desc.length - 1)}
                  </span>
                ) : (
                  <span>...</span>
                )}
              ...
              <button
                className="component_desc_readmore"
        ...
        {eventData.registerURL && content === "more" && (
          <button className="component_desc_btn">
            <a href={eventData.registerURL}>Register Now</a>
          </button>
        )}
      </div>
    </div>
